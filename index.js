...
function uppercase(userAnswer, word){
    //what is word in all capial letters
    //200 points
...
function multiplication(userAnswer, num1,num2){
    //what is num1 multiplicated by num2
    //300 points
...
//if someone was born in birthYear and already has had their birhday
//this year, how old are they?
//300 points
...
function larger(userAnswer,num1,num2){
    // which of ${number4} and ${number5}
    // is larger (if they're the same then pick that number)?
  // 300 pts
...
(function(){
    ...
    function validate(userResponse){
        const answer = process[currentQuestion].validator.apply(window,
            [userResponse,...(process[currentQuestion].meta?
                process[currentQuestion].meta : [])]);
                    let response = ""; 
                    ...
                    if(currentQuestion >= process.length){
                        document.getElementById("container").innerHTML = `<h1>Congrats! 
                         You finished. You got ${getScore()} points!</h1>`;
                         return;
                    }
                    ...
                    const process = [
                        ...
                        {
                            question: `what is the "${word1}" in all capital letters?`,
                            meta:[word1],
                            validator: uppercase,
                            points: 200
                        },
                        ...
                        {
                            question: `what is ${number2} multiplicated by ${number3}?`,
                            meta: [number2,number3],
                            validator: multiplication,
                            points: 300
                        },
                        {
                            question: `If someone was born in ${birthYear} and already has had
                            their birthday this year, how old are they(assuming it's 
                            ${currentYear})?`,
